...
pub fn alloc_1d(elem: Offset, len: u32) -> Result<Value, String> {
...
pub fn aref(a: Value, i: u32) -> Result<Value, String> {
    if i >= len(a) {
        return Err(format!("BoundsError: array index {} of {}", i + 1, len(a)));
    }
    memory::get(mem_of(a), offset_of(a) + i)
}

/// Write element `i` (0-based; `jl_arrayset` over `memoryrefset`).
pub fn aset(a: Value, i: u32, v: Value) -> Result<(), String> {
    if i >= len(a) {
        return Err(format!("BoundsError: array index {} of {}", i + 1, len(a)));
    }
...
/// barrier.
pub fn grow_end(a: Value, inc: u32) -> Result<(), String> {
    let n = len(a);
    let newnrows = n
        .checked_add(inc)
        .ok_or("invalid Array size: too large for system address width")?;
    ...
    if reqmaxsize > oldmaxsize {
        ...
        let newmaxsize =
            u32::try_from(newmaxsize).map_err(|_| "invalid Array size: too large".to_string())?;
...
pub fn del_end(a: Value, dec: u32) -> Result<(), String> {
    let n = len(a);
    if n < dec {
        return Err(format!("BoundsError: cannot delete {} of {} elements", dec, n));
    }
    let n = n - dec;
    set_len(a, n);
    let off = offset_of(a);
...
pub fn push(a: Value, v: Value) -> Result<(), String> {
