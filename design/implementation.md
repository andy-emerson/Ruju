...
`Int`→`Int64`) — currently **126/126, 0 known divergences** (engine slices
1–3, 2026-07; slice 2 added `:452–458`, the invariant-position union family
through `Ref`, incl. shared `S` across union arms; slice 3 added `:70,
79–80, 85–86, 632` — the typevar-count vararg tranche, `NTuple` ≈
`Tuple{Vararg{T,N}}`). History: the unbounded-varargs slice added 19 cases
