...
| **subtype expressibility** | landed 2026-07 (oracle 53→95): ~~unbounded varargs in tuples~~, ~~two-parameter `Pair` (multi-param invariant/diagonal)~~, ~~curated bounded/diagonal `test_3` expansion~~; ~~fixed-count `Vararg{T,N}` (expansion at construction)~~; remaining: typevar-count `Vararg{T,N}` (the `BOUND` kind), `Type{T}` kinds — bounded slices, each unlocking a tranche of `test/subtype.jl` for the oracle | grows the oracle toward the coverage the **engine slice** needs to be measurable; varargs also feeds dispatch |
