...
| **depth in landed subsets** | N-D arrays, `popfirst!`/views, isbits-struct elements; the exception stack (`pop_exception`); nested modules/imports; `BFloat16`, permbox caches (findings 1, 9); the `AbstractArray` tower and exception field metadata (findings 22, 28, audit 2026-07) | pulled in by demand from the two gates above |
