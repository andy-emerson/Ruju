...
| **Subtype engine** | type-depth | XL | W3–W6 | oracle | staged plan in `design/research/research-subtype-engine.md`: slice 1 (union-state machine + drivers) heals both pre-mapped divergences; `Intersect` defers (no oracle case needs it); typevar-`N` separable. **Demoted from research-grade to hard engineering.** First commit: the subtype-env rooting fix |
...
| **Real `CodeInfo` (M2)** | front-end | L *(was XL)* | W3–W6 | build-time pipeline; interpreter completeness | decision D1: build-time pre-lowering; serialized-`CodeInfo` loader + full lowered statement set in `interp.rs`; `frontend.rs` kept as dev convenience |
...
| M2 | end W7 | **Real `CodeInfo`** *(redefined 2026-07 — decision D1)* | The build-time pipeline (pinned native Julia, offline) pre-lowers source; Ruju loads serialized `CodeInfo` as data and `interp.rs` executes the full lowered statement set. Fidelity is maximal by construction. In-browser `eval` moves to its own post-M5 milestone (M5.5). Size ~L, was XL. |
