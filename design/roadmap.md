...
| **Subtype engine** | type-depth | XL | W3–W6 | oracle | ~~slices 1–2 done 2026-07~~ (rooting fix, the union-decision machine + drivers — both pre-mapped divergences healed on first run — and the `forall_exists_equal` tail with the explosion guards); ~~slice 3 done 2026-07-09~~ (the vararg length algebra: `Loffset`, typevar-`N` `Vararg`, the N-equation; oracle → 126, finding 23 closed); remaining per `design/research/research-subtype-engine.md` §6: slice 4 (`Intersect` — no oracle case needs it yet), 5 (`envout`) |
